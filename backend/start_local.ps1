...
if (Test-Path .env) {
  Get-Content .env | ForEach-Object {
    ...
    $k = $k.Trim()
    if (-not $k) { return }
    # trim surrounding quotes in value if present
    if ($null -ne $v) { $v = $v.Trim().Trim('"').Trim("'") }
    # don't override an already-set env var with empty
...
if ($KillRange) {
  ...
  foreach ($p in 8000..8030) {
    ...
    foreach ($procId in $procs) {
      try {
        Write-Host ("[CLEAN] Killing PID {0} on port {1}" -f $procId, $p) -ForegroundColor Yellow
        Stop-Process -Id $procId -Force -ErrorAction Stop
      } catch {
        $msg = $_.Exception.Message
        Write-Host ("[CLEAN] Failed to kill PID {0} on {1}: {2}" -f $procId, $p, $msg) -ForegroundColor Red
      }
    }
  }
}
